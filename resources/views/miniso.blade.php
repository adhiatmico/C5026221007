<html>
    <link
    href="https://fonts.googleapis.com/css2?family=Inter:wght@300;500;600;700&amp;display=swap"
    rel="stylesheet"
  />

    <title>minisoindonesia | Twitter, Instagram, Facebook, TikTok | Linktree</title>
    <link rel="stylesheet" href="{{asset('Tugas Duplicate/MinisoStyle.css')}}">
<body>
    <div><img class="arumi-logoMiniso" src="https://ugc.production.linktr.ee/lty7BCQOGiyc1yJ9EUXg_46e2471e44bc67ec83280cea1b56a8654">
    <div class="arumi-username">@minisoindonesia <img src="data:image/svg+xml;base64,"></div>
    </div>
    </div>


    <div class="arumi-button-container ">
        <div class="arumi-button">
          <a href="https://vt.tiktok.com/ZSJq2kScP/">
            <p>TIKTOK SHOP MINISO INDONESIA</p>
          </a>
          <div class="arumi-dots">
            <tik>...</tik>
          </div>
        </div>
    </div>

    <div class="arumi-button-container ">
        <div class="arumi-button">
          <a href="https://apps.apple.com/cn/app/id1527238446">
            <p>DOWNLOAD MINISO.ID APP ON IOS</p>
          </a>
          <div class="arumi-dots">
            <tik>...</tik>
          </div>
        </div>
    </div>

    <div class="arumi-button-container ">
        <div class="arumi-button">
          <a href="https://apps.apple.com/cn/app/id1527238446">
            <p>DOWNLOAD MINISO.ID APP ON ANDROID</p>
          </a>
          <div class="arumi-dots">
            <tik>...</tik>
          </div>
        </div>
    </div>


    <div class="arumi-button-container ">
        <div class="arumi-button">
          <a href="https://linktr.ee/minisoindonesiastore">
            <p>Buy Online from Miniso stores (Jabodetabek area)</p>
          </a>
          <div class="arumi-dots">
            <tik>...</tik>
          </div>
        </div>
    </div>

    <div class="arumi-button-container ">
        <div class="arumi-button">
          <a href="https://linktr.ee/minisoindonesiastore1">
            <p>Buy Online from Miniso stores (Jawa, Bali, Lombok areas)</p>
          </a>
          <div class="arumi-dots">
            <tik>...</tik>
          </div>
        </div>
    </div>

    <div class="arumi-button-container ">
        <div class="arumi-button">
          <a href="https://linktr.ee/minisoindonesiastore2">
            <p>Buy Online from Miniso stores (Sulawesi, Maluku, Kalimantan areas)</p>
          </a>
          <div class="arumi-dots">
            <tik>...</tik>
          </div>
        </div>
    </div>

    <div class="arumi-button-container ">
        <div class="arumi-button">
          <a href="https://linktr.ee/minisoindonesiastore3">
            <p>Buy Online from Miniso stores (Sumatera area)</p>
          </a>
          <div class="arumi-dots">
            <tik>...</tik>
          </div>
        </div>
    </div>

    <div class="arumi-button-container ">
        <div class="arumi-button">
          <a href="https://linktr.ee/minisoonlineid">
            <p>Shop Miniso products in e-commerce (Presented by @minisoonlineid)</p>
          </a>
          <div class="arumi-dots">
            <tik>...</tik>
          </div>
        </div>
    </div>

    <div class="arumi-button-container ">
        <div class="arumi-button">
          <a href="http://tiktok.com/@minisoindonesia">
            <p>Follow Miniso Indonesia on TikTok</p>
          </a>
          <div class="arumi-dots">
            <tik>...</tik>
          </div>
        </div>
    </div>

    <div class="arumi-button-container ">
        <div class="arumi-button">
          <a href="https://instagram.com/minisoindo">
            <p>Follow Miniso Indonesia on Instagram</p>
          </a>
          <div class="arumi-dots">
            <tik>...</tik>
          </div>
        </div>
    </div>

    <div class="arumi-bottom">
        <div>
            <img class="arumi-botpic" src="https://logos-world.net/wp-content/uploads/2022/12/Linktree-Logo.jpg">
        </div>
    </div>



</body>

</html>
